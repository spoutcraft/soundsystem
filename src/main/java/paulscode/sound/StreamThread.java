...
/**
 * The StreamThread class is used to process all streaming sources. This thread starts out asleep, and it sleeps when all streaming sources are finished playing, so it is necessary to call
 * interrupt() after adding new streaming sources to the list. <br><br><b><i>SoundSystem License:</b></i><br><b><br> You are free to use this library for any purpose, commercial or otherwise. You
 * may modify this library or source code, and distribute it any way you like, provided the following conditions are met: <br>1) You may not falsely claim to be the author of this library or any
 * unmodified portion of it. <br>2) You may not copyright this library or a modified version of it and then sue me for copyright infringement. <br>3)If you modify the source code, you must clearly
 * document the changes made before redistributing the modified source code, so other users know it is not the original code. <br>4) You are not required to give me credit for this library in any
 * derived work, but if you do, you must also mention my website: http://www.paulscode.com <br>5) I the author will not be responsible for any damages (physical, financial, or otherwise) caused by
 * the use if this library or any part of it. <br>6) I the author do not guarantee, warrant, or make any representations, either expressed or implied, regarding the use of this library or any part of
 * it. <br><br>Author: Paul Lamb <br>http://www.paulscode.com</b>
 */
public class StreamThread extends SimpleThread {
									...
									*/
									...
									if (src.toLoop) {
										...
										if (!src.playing()) {
											...
											// Check if the source is currently in the process of fading out.
											if (src.checkFadeOut()) {
												// Source is fading out. Keep looping until it finishes.
												src.preLoad = true;
											} else {
												// Source is not fading out. If there is another sound in the sequence, switch to it before replaying.
												src.incrementSoundSequence();
										...
										if (!src.playing()) {
											...
											// Check if the source is currently in the process of fading out.
											if (!src.checkFadeOut()) {
												// Source is not fading out. Play anything else that is in the sound sequence queue.
	...
	public void watch(Source source) {
		...
		// Make sure no one else is accessing the list of sources.
		synchronized (listLock) {
			// Any currently watched source which is null or playing on the same channel as the new source should be stopped and removed from the list.
			iter = streamingSources.listIterator();
			while (iter.hasNext()) {
				src = iter.next();
				if (src == null) {
