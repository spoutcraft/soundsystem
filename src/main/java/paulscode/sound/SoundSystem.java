...
/**
 * The SoundSystem class is the core class for the SoundSystem library. It is capable of interfacing with external sound library and codec library plugins. This core class is stripped down to give
 * it a smaller memory footprint and to make it more customizable. This library was created to provide a simple, common interface to a variety of 3rd-party sound and codec libraries, and to simplify
 * switching between them on the fly. If no external plugins are loaded, this core class by itself is only capable of playing MIDI files. Specific implementations (such as SoundSystemJPCT) will
 * extend this core class. They will automatically link with popular external plugins and provide extra methods for ease of use. There should be only one instance of this class in any program!  The
 * SoundSystem can be constructed by defining which sound library to use, or by allowing SoundSystem to perform its own library compatibility checking. See {@link paulscode.sound.SoundSystemConfig
 * SoundSystemConfig} for information about changing default settings and linking with external plugins. <br><br><b><i>SoundSystem License:</b></i> <br><b><br>You are free to use this library
 * for any purpose, commercial or otherwise. You may modify this library or source code, and distribute it any way you like, provided the following conditions are met: <br>1) You may not falsely
 * claim to be the author of this library or any unmodified portion of it. <br>2) You may not copyright this library or a modified version of it and then sue me for copyright infringement. <br>3) If
 * you modify the source code, you must clearly document the changes made before redistributing the modified source code, so other users know it is not the original code. <br>4) You are not required
 * to give me credit for this library in any derived work, but if you do, you must also mention my website: http://www.paulscode.com <br>5) I the author will not be responsible for any damages
 * (physical, financial, or otherwise) caused by the use if this library or any part of it. <br>6) I the author do not guarantee, warrant, or make any representations, either expressed or implied,
 * regarding the use of this library or any part of it. <br><br>Author: Paul Lamb <br>http://www.paulscode.com</b>
 */
public class SoundSystem {
	 ...
	 *
	 * @param libraryClass Library to use. See {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} for information about choosing a sound library.
	 */
	...
	/**
	 * Links with any default libraries or codecs should be made in this method. This method is empty in the core SoundSystem class, and should be overridden by classes which extend SoundSystem. See
	 ...
	 *
	 * @param libraryClass Library to initialize. See {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} for information about choosing a sound library.
	 */
	...
	/**
	 * Wakes up the Command Thread to process commands. This method should be used if there is a need to call methods 'ManageSources' and 'CommandQueue' externally. In most cases, this method will
	 * not be needed, since SoundSystem automatically wakes the Command Thread every time commands are placed into either the ManageSources queue or CommandQueue to be processed.
	 */
	 ...
	 * recognizes URLs. If the file is located within the compiled JAR, the package in which sound files are located may be set by calling SoundSystemConfig.setSoundFilesPackage().
	 *
	 ...
	 * an online location. If the file is online, filename must begin with "http://", since that is how SoundSystem recognizes URL paths. If the file is located within the compiled JAR, the package
	 * in which sound files are located may be set by calling SoundSystemConfig.setSoundFilesPackage(). This method has no effect on non-streaming sources.
	 *
	 ...
	 * located within the JAR or at an online location. If the file is online, filename must begin with "http://", since that is how SoundSystem recognizes URL paths. If the file is located within
	 * the compiled JAR, the package in which sound files are located may be set by calling SoundSystemConfig.setSoundFilesPackage(). If the filename parameter is null or empty, the specified source
	 * will simply fade out and stop. The milliseconds parameter must be non-negative or zero. This method will remove anything that is currently in the specified source's list of queued sounds that
	 * would have played next when the current sound finished playing. This method may only be used for streaming and MIDI sources.
	 *
	 * @param sourcename Name of the source to fade out.
	 * @param filename Name of a sound file to play next, or null for none.
	 * @param milis Number of milliseconds the fadeout should take.
	 */
	...
	/**
	 * Fades out the volume of whatever the specified source is currently playing, then begins playing the specified file at the source's previously assigned volume level. If the url parameter is
	 * null or empty, the specified source will simply fade out and stop. The third parameter 'identifier' should look like a filename, and it must have the correct extension so SoundSystem knows
	 * what codec to use for the file referenced by the URL instance. The milliseconds parameter must be non-negative or zero. This method will remove anything that is currently in the specified
	 ...
	 * by calling SoundSystemConfig.setSoundFilesPackage(). The milliseconds parameters must be non-negative or zero. This method will remove anything that is currently in the specified source's list
	 ...
	 * @param filename Name of a sound file to play next, or null for none.
	 * @param milisOut Number of milliseconds the fadeout should take.
	 * @param milisIn Number of milliseconds the fadein should take.
	 */
	 ...
	 * SoundSystem knows what codec to use for the file referenced by the URL instance. The milliseconds parameters must be non-negative or zero. This method will remove anything that is currently in
	...
	/**
	 * Makes sure the current volume levels of streaming sources and MIDI are correct. This method is designed to help reduce the "jerky" fading behavior that happens when using some library and
	 * codec pluggins (such as LibraryJavaSound and CodecJOrbis). This method has no effect on normal "non-streaming" sources. It would normally be called somewhere in the main "game loop".
	 * IMPORTANT: To optimize frame-rates, do not call this method for every frame. It is better to just call this method at some acceptable "granularity" (play around with different granularities to
	...
	/**
	 * Creates a new streaming source. The file may either be located within the JAR or at an online location. If the file is online, filename must begin with "http://", since that is how
	 * SoundSystem recognizes URL paths. If the file is located within the compiled JAR, the package in which sound files are located may be set by calling SoundSystemConfig.setSoundFilesPackage().
	 *
	 * @param priority Setting this to true will prevent other sounds from overriding this one.
	 * @param sourcename A unique identifier for this source. Two sources may not use the same sourcename.
	...
	/**
	 * Creates a temporary source and streams it. After the source finishes playing, it is removed. The third parameter 'identifier' should look like a filename, and it must have the correct
	 * extension so SoundSystem knows what codec to use for the file referenced by the URL instance. Returns a randomly generated name for the new source. NOTE: to make a source created by this
	 * method permanant, call the setActive() method using the return value for sourcename.
	 *
	 * @param priority Setting this to true will prevent other sounds from overriding this one.
	 * @param url URL handle to the sound file to stream at this source.
	 * @param identifier Filename/identifier of the file referenced by the URL.
	 ...
	 * created with the rawDataStream() method. NOTE:  Be carefull how much data you send to a source to stream. The data will be processed at playback speed, so if you queue up 1 hour worth of
	 * data, it will take 1 hour to play (not to mention hogging a ton of memory). To clear out all queued data from the source, use the flush() method. Also note: if there is a break in the data
	 * stream, you will hear clicks and studders, so ensure that the data flow is steady.
	 *
	...
	/**
	 * Switches to the specified library, loosing all sources. This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the
	 * newLibrary() method instead.
	 *
	 * @param libraryClass Library to use. See {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} for information about chosing a sound library.
	 */
	...
	/**
	 * Saves the specified sample data, under the specified identifier. This identifier can be later used in place of 'filename' parameters to reference the sample data. This method is used
	 * internally by SoundSystem for thread synchronization, and it can not be called directly - please use the loadSound() method instead.
	 *
	...
	/**
	 * Fades out the volume of whatever the specified source is currently playing, then begins playing the specified file at the source's previously assigned volume level. If the filenameURL
	 * parameter is null or empty, the specified source will simply fade out and stop. The milliseconds parameter must be non-negative or zero. This method will remove anything that is currently in
	 * the specified source's list of queued sounds that would have played next when the current sound finished playing. This method may only be used for streaming and MIDI sources. This method is
	 * used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the fadeOut() method instead.
	 *
	 * @param sourcename Name of the source to fade out.
	...
	/**
	 * Fades out the volume of whatever the specified source is currently playing, then fades the volume back in playing the specified file. Final volume after fade-in completes will be equal to the
	 * source's previously assigned volume level. The filenameURL parameter may not be null or empty. The milliseconds parameters must be non-negative or zero. This method will remove anything that
	 * is currently in the specified source's list of queued sounds that would have played next when the current sound finished playing. This method may only be used for streaming and MIDI sources.
	 * This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the fadeOutIn() method instead.
	 *
	 * @param sourcename Name of the source to fade out/in.
	 * @param filenameURL Filename/URL of a sound file to play next, or null for none.
	 * @param milisOut Number of milliseconds the fadeout should take.
	 ...
	 * codec pluggins (such as LibraryJavaSound and CodecJOrbis). This method has no effect on normal "non-streaming" sources. It would normally be called somewhere in the main "game loop".
	 * IMPORTANT: To optimize frame-rates, do not call this method for every frame. It is better to just call this method at some acceptable "granularity" (play around with different granularities to
	 * find what sounds acceptable for a particular situation). This method is used internally by SoundSystem for thread synchronization, and it can not be called directly - please use the
	 * checkFadeVolumes() method instead.
	 */
	...
	/**
	 * Creates a temporary source and either plays or streams it. After the source finishes playing, it is removed. This method is used internally by SoundSystem for thread synchronization, and it
	 * can not be called directly - please use the quickPlay() method instead.
	 *
	 * @param priority Setting this to true will prevent other sounds from overriding this one.
	 * @param toStream Whether or not to stream the source.
	 * @param toLoop Whether or not to loop the source.
	...
	/**
	 * Set a source's priority factor. A priority source will not be overriden when too many sources are playing at once. This method is used internally by SoundSystem for thread synchronization,
	 * and it can not be called directly - please use the setPriority() method instead.
	 *
	 * @param sourcename Identifier for the source.
	 * @param pri Setting this to true makes this source a priority source.
	 */
	 ...
	 * source to active does not play it. It is also important to note that a looping inactive source will not be removed as long as it keeps playing. This method is used internally by SoundSystem
	 * for thread synchronization, and it can not be called directly - please use the setTemporary() method instead.
	 *
	 * @param sourcename Identifier for the source.
	...
	/**
	 * Turns the listener counterclockwise by "angle" radians around the y-axis, relative to the current angle. This method is used internally by SoundSystem for thread synchronization, and it can
	 * not be called directly - please use the turnListener() method instead.
	 *
	 * @param angle radian offset.
	 */
