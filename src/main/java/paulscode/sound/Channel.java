 ...
 * are played back. Channels can be either streaming channels or normal (non-streaming) ones. For consistent naming conventions, each sub-class should have the name prefix "Channel". <br><br><b><i>
 * SoundSystem License:</b></i> <br><b><br>You are free to use this library for any purpose, commercial or otherwise. You may modify this library or source code, and distribute it any way you like,
 * provided the following conditions are met: <br>1) You may not falsely claim to be the author of this library or any unmodified portion of it. <br>2) You may not copyright this library or a
 * modified version of it and then sue me for copyright infringement. <br>3) If you modify the source code, you must clearly document the changes made before redistributing the modified source code,
 * so other users know it is not the original code. <br>4) You are not required to give me credit for this library in any derived work, but if you do, you must also mention my website:
...
public class Channel {
	...
	/**
	 * Global identifier for the type of channel (normal or streaming). Possible values for this variable can be found in the {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} class.
	 */
	...
	/**
	 * Cumulative counter of the buffers played, then unqueued.
	 */
	public int buffersUnqueued = 0;

	/**
	 * Constructor:  Takes channelType identifier as a parameter. Possible values for channel type can be found in the {@link paulscode.sound.SoundSystemConfig SoundSystemConfig} class.
	 *
	...
	/**
	 * Plays the next queued byte[] buffer. This method is run from the separate. {@link paulscode.sound.StreamThread StreamThread}.
	 *
