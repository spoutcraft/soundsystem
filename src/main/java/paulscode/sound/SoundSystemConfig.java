...
/**
 * The SoundSystemConfig class is used to access global sound system settings, and to link with external plugins. All members of this class are static. SoundSystemConfig is sort of a "catch all"
 * configuration class, so if you are not sure where to find something in the SoundSystem library, this is probably a good place to start. <br><br><b><i>SoundSystem License:</b></i> <br><b><br>
 ...
 * <br>1) You may not falsely claim to be the author of this library or any unmodified portion of it. <br>2) You may not copyright this library or a modified version of it and then sue me for
 * copyright infringement. <br>3) If you modify the source code, you must clearly document the changes made before redistributing the modified source code, so other users know it is not the original
 * code. <br>4) You are not required to give me credit for this library in any derived work, but if you do, you must also mention my website: http://www.paulscode.com <br>5) I the author will not be
 * responsible for any damages (physical, financial, or otherwise) caused by the use if this library or any part of it. <br>6) I the author do not guarantee, warrant, or make any representations,
 * either expressed or implied, regarding the use of this library or any part of it. <br><br>Author: Paul Lamb <br>http://www.paulscode.com</b>
 */
public class SoundSystemConfig {
	/**
	 * Lock object used to synchronize the three threads used by SoundSystem. Synchronize on this any time you manually manipulate a Source's properties.
	 */
	...
	/**
	 * Global identifier for no attenuation. Attenuation is how a source's volume fades with distance. When there is no attenuation, a source's volume remains constant regardless of distance.
	 */
	...
	/**
	 * Handle to the message logger. The default logger can be changed by overriding the {@link paulscode.sound.SoundSystemLogger SoundSystemLogger} class and calling the setLogger() method (must be
	...
	/**
	 * MIDI device to try using as the Synthesizer. May be the full name or part of the name. If this String is empty, the default Synthesizer will be used, or one of the common alternate
	 * synthesizers if the default Synthesizer is unavailable.
	 */
	 ...
	 * messages are handled. First, the {@link paulscode.sound.SoundSystemLogger SoundSystemLogger} class should be extended and methods overridden to change how messages are handled. Then, the
	 ...
	 * non-streaming channels will be available. If unable to create the number of channels specified, SoundSystem will create as many as possible. NOTE: Some sound library plugins may require the
	 ...
	 * channels will be available. If unable to create the number of channels specified, SoundSystem will create as many as possible. NOTE: Some sound library plugins may require the total number of
	...
	/**
	 * Sets the doppler factor, for determining Doppler Effect scale. Use this method BEFORE instantiating the SoundSystem. To change the Doppler factor after the SoundSystem is instantiated, use
	 * the SoundSystem.changeDopplerFactor method instead.
	 *
	...
	/**
	 * Sets the number of buffers used for each streaming source. Slow codecs may require this number to be greater than 2 to prevent audio skipping during playback.
	 *
	...
	public static synchronized void setCodec(String extension, Class iCodecClass) throws SoundSystemException {
		...
		// Let SoundSystem know if this is a MIDI codec, so it won't use javax.sound.midi anymore.
	...
	// We don't know what Class parameter 'c' is, so we will ignore the warning message "unchecked call to getMethod".
	...
	private static class Codec {
		...
		public Codec(String extension, Class iCodecClass) {
			...
			if (extension != null && extension.length() > 0) {
				// We are only interested in the file extension. The filename can begin with whatever.
				extensionRegX = ".*";
				String c;
		...
		private void instantiationErrorMessage() {
			errorMessage("Unrecognised ICodec implementation in method " + "'getInstance'. Ensure that the implementing " + "class has one public, parameterless constructor.");
		}
	}
}
