 ...
 * <br><br><b><i>SoundSystem License:</b></i> <br><b><br>You are free to use this library for any purpose, commercial or otherwise. You may modify this library or source code, and distribute it
 * any way you like, provided the following conditions are met: <br>1) You may not falsely claim to be the author of this library or any unmodified portion of it. <br>2) You may not copyright this
 * library or a modified version of it and then sue me for copyright infringement. <br>3) If you modify the source code, you must clearly document the changes made before redistributing the modified
 * source code, so other users know it is not the original code. <br>4) You are not required to give me credit for this library in any derived work, but if you do, you must also mention my website:
 * http://www.paulscode.com <br>5) I the author will not be responsible for any damages (physical, financial, or otherwise) caused by the use if this library or any part of it. <br>6) I the author
 * do not guarantee, warrant, or make any representations, either expressed or implied, regarding the use of this library or any part of it. <br><br>Author: Paul Lamb <br>http://www.paulscode.com</b>
 */
public class MidiChannel implements MetaEventListener {
	 ...
	 * empty, playback will simply fade out and stop. The milliseconds parameter must be non-negative or zero. This method will remove anything that is currently in the list of queued MIDI sequences
	 ...
	 * currently in the list of queued MIDI sequences that would have played next when current playback finished.
	 *
	 * @param filenameURL MIDI file to play next, or null for none.
	...
	public void meta(MetaMessage message) {
		if (message.getType() == END_OF_TRACK) {
			...
			if (toLoop) {
				// Looping. Check if playback is in the process of fading out.
				if (!checkFadeOut()) {
					// Not fading out, progress to the next MIDI sequence if any are queued.
					if (!incrementSequence()) {
						try {
							// Rewind to the beginning.
							sequencer.setMicrosecondPosition(0);
	...
	/**
	 * First attempts to load the specified "override MIDI synthesizer" if one was defined. If none was defined or unable to use it, then attempts to load the default synthesizer. If that fails,
	 * then other common synthesizers are attempted. If none can be loaded, then MIDI is not possible on this system.
	 */
