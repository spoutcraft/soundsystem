 ...
 * is the template class which is extended for each specific library. This class is also used by the "No Sound" library to represent a mute source. <br><br><b><i>SoundSystem
 * License:</b></i><br><b><br> You are free to use this library for any purpose, commercial or otherwise. You may modify this library or source code, and distribute it any way you like, provided the
 * following conditions are met: <br>1) You may not falsely claim to be the author of this library or any unmodified portion of it. <br>2) You may not copyright this library or a modified version of
 * it and then sue me for copyright infringement. <br>3) If you modify the source code, you must clearly document the changes made before redistributing the modified source code, so other users know
 * it is not the original code. <br>4) You are not required to give me credit for this library in any derived work, but if you do, you must also mention my website: http://www.paulscode.com <br>5) I
 * the author will not be responsible for any damages (physical, financial, or otherwise) caused by the use if this library or any part of it. <br>6) I the author do not guarantee, warrant, or make
 * any representations, either expressed or implied, regarding the use of this library or any part of it. <br><br>Author: Paul Lamb <br>http://www.paulscode.com</b>
 */
public class Source {
	/**
	 ...
	 */
	public Source(boolean priority, boolean toStream, boolean toLoop, String sourcename, FilenameURL filenameURL, SoundBuffer soundBuffer, float x, float y, float z,
	...
	/**
	 * Fades out the volume of whatever this source is currently playing, then begins playing the specified filename at the source's previously assigned volume level.  If the filename parameter is
	 * null or empty, the source will simply fade out and stop.  The milliseconds parameter must be non-negative or zero.  This method will remove anything that is currently in the list of queued
	 * sounds that would have played next when the current sound finished playing.  This method has no effect on non-streaming sources.
	 *
	 ...
	 * previously assigned volume level. The filenameURL parameter may not be null or empty. The milliseconds parameters must be non-negative or zero. This method will remove anything that is
	 * currently in the list of queued sounds that would have played next when the current sound finished playing. This method has no effect on non-streaming sources.
	 *
	 * @param filenameURL Filename/URL of the sound file to play next, or null for none.
	...
	/**
	 * Resets this source's volume if it is fading out or in. Returns true if this source is currently in the process of fading out. When fade-out completes, this method transitions the source to
	 * the next sound in the sound sequence queue if there is one. This method has no effect on non-streaming sources.
	 *
	...
	/**
	 * Removes the next filename/URL from the sound sequence queue and assigns it to this source. This method has no effect on non-streaming sources.  This method is used internally by SoundSystem,
	 * and it is unlikely that the user will ever need to use it.
	 *
	...
	/**
	 * Reads in initial buffers of data from the next sound in the sound sequence queue, to reduce lag when the transition occurs. This method has no effect on non-streaming sources. This method is
	...
	/**
	 * Sets whether or not this source is a priority source. A priority source will not be overwritten by another source if there are no channels available to play on.
	 *
	 ...
	 * @param temporary Whether or not to remove this source after it finishes playing.
	 */
	public void changeSource(boolean priority, boolean toStream, boolean toLoop, String sourcename, FilenameURL filenameURL, SoundBuffer soundBuffer, float x, float y, float z, int attModel,
