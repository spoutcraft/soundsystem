 ...
 * should extend this class and override the necessary methods. For consistent naming conventions, each sub-class should have the name prefix "Library".
 *
 * This class may also be used as the "No Sound Library" (i.e. silent mode) if no other audio libraries are supported by the host machine, or to mute all sound. <br><br><b><i>SoundSystem
...
public class Library {
	...
	/**
	 * Constructor: Instantiates the source map and listener information. NOTES: The 'super()' method should be at the top of constructors for all extended classes. The varriable 'libraryType'
	 * should be given a new value in the constructors for all extended classes.
	 */
	...
	/**
	 * Removes a pre-loaded sound from memory. This is a good method to use for freeing up memory after a large sound file is no longer needed. NOTE: the source will remain in memory after this
	 * method has been called, for as long as the sound is attached to an existing source.
	 *
